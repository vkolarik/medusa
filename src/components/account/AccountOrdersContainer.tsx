...
export const AccountOrdersContainer: FC<{ data: IAccountOrders[] }> = ({ data }) => {
      ...
      {data.map((item: IAccountOrders, key: number) => {
        ...
        return <div key={key}>
          <div className="flex justify-between lg:py-2 py-1">
            <h2 className="font-medium lg:text-[16px] md:text-[15px] text-[14px]">Objednáno: {date}</h2>
            <h2 className="font-medium lg:text-[16px] md:text-[15px] text-[14px]">Celkem: {calculateTotalPrice(products.map(p => p.item))} Kč</h2>
          </div>
          ...
          <div className="grid lg:grid-cols-2 lg:gap-x-8 lg:gap-y-0 gap-y-5 lg:py-8 pb-8 pt-4 border-t border-lightGrey">
            {products.map((product: IProductSummary, productKey: number) => {
              ...
              return <ProductSummary key={productKey}
                item={item}
